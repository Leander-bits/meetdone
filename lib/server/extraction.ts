...
Do not fill missing information, assume consensus, infer owners or deadlines, convert vague agreement to a decision, convert suggestions to commitments, or fabricate evidence. Use null/missing/not_discussed/not_mentioned when unsupported. A topic mention is not sufficient coverage. A speaker counts only if they express relevant input on their required topic. Presence or being mentioned is present_no_opinion. For each required decision distinguish not_discussed, discussed_not_decided, decided; decided requires an explicit outcome.
