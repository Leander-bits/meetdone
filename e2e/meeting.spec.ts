...
test("demo moves from four blockers to a ready meeting and persisted summary", async ({
  ...
  await page.locator("summary").filter({ hasText: "Load Demo Scenario" }).click();
  await page.getByRole("button", { name: /Scenario B/ }).click();
  await page.getByRole("button", { name: "Use Demo Analysis" }).click();
  await page.getByRole("button", { name: "Prepare to End Meeting", exact: true }).click();
  await expect(page.locator("#meeting-status").getByRole("status")).toContainText("Ready to End");
...
test("exception reason is required and ending preserves blocked readiness", async ({ page }) => {
  ...
  expect(saved.completionCheck.readiness).toBe("BLOCKED");
});
...
test("custom transcripts are saved and cannot run fixture analysis", async ({ page }) => {
  ...
  await page.locator("summary").filter({ hasText: "Load Demo Scenario" }).click();
  await page.getByRole("button", { name: "Use Demo Analysis" }).click();
  await expect(page.getByText(/Select a demo scenario first/)).toBeVisible();
  await page.reload();
  ...
  await page.getByRole("button", { name: "Prepare to End Meeting", exact: true }).click();
  await expect(page.getByRole("button", { name: "End with Exception", exact: true })).toHaveCount(
    0,
  );
});
...
test("create from a template, edit requirements, and recover invalid storage", async ({ page }) => {
  ...
  await page.getByRole("combobox", { name: "Template", exact: true }).selectOption("retro");
  await page.getByLabel("Meeting name").fill("Delivery retrospective");
  await page
    .getByRole("dialog")
...
test("action repairs reduce blockers and non-deferrable follow-ups keep the decision blocked", async ({
  ...
  await page.getByRole("button", { name: "Prepare to End Meeting", exact: true }).click();
  await expect(page.getByRole("article")).toHaveCount(2);
  await expect(page.locator("#meeting-status").getByRole("status")).toBeVisible();
  await page.reload();
  await page
    .locator("summary")
    .filter({ hasText: /^Action Items/ })
